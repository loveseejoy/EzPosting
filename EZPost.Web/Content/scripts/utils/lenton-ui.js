...
Loading = function (bool, text) {
    ...
    if (!!text) {
        ...
        top.$("#loading_manage").html("Loading…");
    }
...
//------------------------------------------------------------------------------------------------------------
checkedSelected = function (dataArray, isMuilteSelected) {
    var isOK = true;
    if (dataArray.length == 0) {
        isOK = false;
        dialogMsg('请选择一条数据！', 0);
    } else if (isMuilteSelected == false && dataArray.length > 1) {
        isOK = false;
        dialogMsg('只能选择一条数据！', 0);
    }
    return isOK;
}
