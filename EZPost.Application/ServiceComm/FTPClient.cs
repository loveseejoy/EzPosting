...
namespace EZPost.ServiceComm
{
    ...
    public class FtpClient
    {
        ...
        /// /// <param name="remotePath">链接的目录(前面不需要加/，直接写目录如 R32/OK)</param>
        public FtpClient(Uri FtpUri, string strUserName, string strPassword,string remotePath=null)
        {
        ...
        public FtpClient(Uri FtpUri, string strUserName, string strPassword, WebProxy objProxy)
        {
            this._Uri = new Uri(FtpUri.GetLeftPart(UriPartial.Authority));
            _DirectoryPath = FtpUri.AbsolutePath;
            if (!_DirectoryPath.EndsWith("/"))
