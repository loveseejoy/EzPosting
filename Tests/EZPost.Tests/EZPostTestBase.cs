...
namespace EZPost.Tests
{
    public abstract class EZPostTestBase : AbpIntegratedTestBase<EZPostTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected EZPostTestBase()
        {
        ...
        protected void UsingDbContext(Action<EZPostDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<EZPostDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<EZPostDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<EZPostDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<EZPostDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EZPostDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<EZPostDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<EZPostDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<EZPostDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<EZPostDbContext, Task<T>> func)
        {
            T result;
